    ...
    */
    ...
    function FlickrProcessItems( albumIdx, processLocationHash, imageID, setLocationHash) {
      ...
      var tId = setTimeout( function() {
        // workaround to handle JSONP (cross-domain) errors
        PreloaderHide();
        nanoAlert('Could not retrieve data...');
      }, 60000 );
      jQuery.getJSON(url, function(data, status, xhr) {
        clearTimeout(tId);
        PreloaderHide();
...
//        ch='';
    ...
    function OpenInternalViewer( imageIdx ) {
      ...
      // build image toolbar
      var sTB='<div  style="visibility:'+(G.O.viewerToolbar.display ? "visible" : "hidden")+';"><div class="">';
      ...
      switch( e ) {
        ...
        default:
          if( e.indexOf('custom') == 0 ) {
            r='<div '+e+' nGEvent">' + (typeof G.O.fnImgToolbarCustInit == 'function' ? G.O.fnImgToolbarCustInit(e) : '') + '</div>';
          }
          break;
      }
      return r;
    ...
    function nanoAlert( msg, verbose ) {
      ...
      if( G.$E.conConsole != null ) {
        ...
        if( verbose == false ) {
          ...
          G.$E.conConsole.append('<p> '+msg+ ' ['+G.baseEltID+']</p>');
        }
    ...
    function nanoConsoleLog( msg ) {
      if (window.console) { console.log(' ' + msg + ' ['+G.baseEltID+']'); }
    };
